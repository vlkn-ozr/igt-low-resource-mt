...
The `scripts/` directory contains Python utilities for working with TRmorph:

- `analysis.py` - Analyze Turkish words from a file and save results as JSON
- `disambiguate.py` - Analyze and disambiguate tokens using a trained model
- `segment-filter.py` - Filter segmentation results to keep only valid segments
- `stats.py` - Calculate statistics from analysis JSON files
- `stats_duplicate.py` - Remove duplicate unanalyzed words from statistics
- `top_max_analysis.py` - Find words with the most analyses

### Usage Examples

```bash
# Analyze words from a file
python scripts/analysis.py input.txt output.json

# Disambiguate tokens
echo "okudum" | python scripts/disambiguate.py -m scripts/1M.m2

# Calculate statistics
python scripts/stats.py output.json output.stats.json

# Find top words with most analyses
python scripts/top_max_analysis.py output.json 10
```

Note: The `1M.m2` model file is required for disambiguation and can be downloaded from:
http://www.let.rug.nl/~coltekin/trmorph/1M.m2
