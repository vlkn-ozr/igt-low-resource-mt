...
class AnalysisStats:
    ...
    def calculate_coverage_stats(self, words, analyses):
        ...
        for word, analysis in zip(words, analyses):
            ...
            if (
                not analysis
                or analysis == "no result"
                or (
                    isinstance(analysis, list)
                    and (
                        "no result" in str(analysis)
                        or any("no result for" in str(item) for item in analysis)
                    )
                )
            ):
        ...
        coverage_percentage = (
            (analyzed_words / total_words * 100.0) if total_words > 0 else 0.0
        )
                ...
                if analyses and isinstance(analyses, list):
                    ...
                    for analysis in analyses:
                        if (
                            ...
                            or analysis == ""
                            or analysis == "+?"
                            or "no result" in str(analysis)
                            or "no result for" in str(analysis)
                        ):
    ...
    def calculate_analyses_stats(self) -> Dict:
        ...
        if analyses_counts:
            return {
                "average_analyses_per_word": round(
                    sum(analyses_counts) / len(analyses_counts), 2
                ),
    ...
    def get_unanalyzed_words(self) -> List[str]:
        """Return a list of words that couldn't be analyzed."""
        unanalyzed = []

        for sentence in self.data["sentences"]:
            for word, analyses in sentence["analyses"].items():
                if not analyses or not isinstance(analyses, list):
                    unanalyzed.append(word)
                else:
                    # Check each analysis in the list
                    has_valid_analysis = True
                    for analysis in analyses:
                        if (
                            not analysis
                            or analysis == ""
                            or analysis == "+?"  # Added condition for "+?" analysis
                            or "no result" in str(analysis)
                            or "no result for" in str(analysis)
                        ):
                            has_valid_analysis = False
    ...
    def save_report(self, output_file: Path = None):
        ...
        print(
            f"Average analyses per word: {report['analyses_stats']['average_analyses_per_word']}"
        )
